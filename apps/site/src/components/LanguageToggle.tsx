...
export default function LanguageToggle({ language, onChange, className = '' }: Props) {
  ...
  return (
    ...
    >
      ...
      <span
        className={`relative inline-block h-5 w-9 rounded-full transition-colors duration-200 group-focus-visible:ring-2 group-focus-visible:ring-[#1a1a1a] group-focus-visible:ring-offset-2 group-focus-visible:ring-offset-[#faf9f6] ${
      ...
      >
        <span
          className={`absolute top-0.5 inline-block h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-200 ${
            language === 'ko' ? 'translate-x-[18px]' : 'translate-x-0.5'
          }`}
        />
      </span>
      <span
